package com.example.waheed.bakingapp.recipedetails;
...
public class RecipeDetailsScreenTest {
    ...
    private Recipe[] recipes = new Gson().fromJson(recipesJson, Recipe[].class);

    private final Recipe RECIPE = recipes[3];
    ...
    @Before
    public void startRecipeDetailsScreenWithStubbedIntent() {
    ...
    @Test
    public void recipeDetailsScreen_showsTheExpectedRecipeSteps() {
        List<RecipeStep> steps = RECIPE.getSteps();

        onView(withId(R.id.recipeStepsRecyclerView))
                .perform(scrollToPosition(steps.size() - 1));
        String expectedStepText = steps.get(steps.size() - 1).getShortDescription();

        // Verify recipe step is displayed on screen
        onView(withItemText(expectedStepText)).check(matches(isDisplayed()));
    }
    ...
    private static final String recipesJson =
            "[\n" +
                    "  {\n" +
                    "    \"id\": 1,\n" +
                    "    \"name\": \"Mocked Nutella Pie\",\n" +
                    "    \"ingredients\": [\n" +
                    "      {\n" +
                    "        \"quantity\": 2,\n" +
                    "        \"measure\": \"CUP\",\n" +
